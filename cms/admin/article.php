<?php
// 파트 A: 설정하기
declare(strict_types = 1);                                      // 엄격한 타입 사용
include '../includes/database-connection.php';                  // 데이터베이스 연결
include '../includes/functions.php';                            // 함수
include '../includes/validate.php';                             // 유효성 검사 함수

$uploads = dirname(__DIR__, 1) . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR; // 업로드 파일 저장소
$file_types = ['image/jpeg', 'image/png', 'image/gif'];         // 허용되는 타입
$file_exts  = ['jpg', 'jpeg', 'png', 'gif'];                    // 허용되는 확장자
$max_size   = 5242880;                                          // 최대 파일 크기

// PHP 코드에 필요한 변수 초기화
$id          = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT); // 아이디 가져오기 + 유효성 검사
$temp        = $_FILES['image']['tmp_name'] ?? '';              // 임시 이미지
$destination = '';                                              // 파일 저장할 위치
$invalid     = '';                                              // 오류 결합 결과

// HTML 페이지에 필요한 변수 초기화
$article = [
    'id'          => $id,
    'title'       => '',
    'summary'     => '',
    'content'     => '',
    'member_id'   => 0,
    'category_id' => 0,
    'image_id'    => null,
    'published'   => false,
    'image_file'  => '',
    'image_alt'   => '',
];                                                             // 기사 데이터

$errors = [
    'warning'    => '',
    'title'      => '',
    'summary'    => '',
    'content'    => '',
    'author'     => '',
    'category'   => '',
    'image_file' => '',
    'image_alt'  => '',
];                                                             // 오류 메시지

// 아이디가 있다면, 페이지는 기사 편집 중이므로 현재 기사 데이터 가져오기
if ($id) {                                                      // 아이디가 있다면
    $sql = "SELECT a.id, a.title, a.summary, a.content,
                   a.category_id, a.member_id, a.image_id, a.published,
                   i.file     AS image_file,
                   i.alt      AS image_alt
              FROM article    AS a
              LEFT JOIN image AS i ON a.image_id = i.id
             WHERE a.id = :id;";                               // 기사를 가져오는 SQL

    $article = pdo($pdo, $sql, [$id])->fetch();                 // 기사 데이터 가져오기

    if (!$article) {                                            // 기사 없으면
        redirect('articles.php', ['failure' => 'Article not found']); // 리디렉션
    }
}

$saved_image = $article['image_file'] ? true : false;           // 이미지가 업로드되었는가?

// 모든 회원과 카테고리 가져오기
$sql = "SELECT id, forename, surname FROM member;";             // 모든 회원 가져오는 SQL
$authors = pdo($pdo, $sql)->fetchAll();                         // 모든 회원 가져오기

$sql = "SELECT id, name FROM category;";                        // 모든 카테고리 가져오는 SQL
$categories = pdo($pdo, $sql)->fetchAll();                      // 모든 카테고리 가져오기

// 파트 B: 폼 데이터 가져오고 유효성 검사하기
if ($_SERVER['REQUEST_METHOD'] === 'POST') {                    // 폼이 제출되었다면

    // 파일이 php.ini 또는 .htaccess에서 지정된 크기보다 더 클 때의 오류 메시지
    $image_error = $_FILES['image']['error'] ?? UPLOAD_ERR_NO_FILE;
    $errors['image_file'] = ($image_error === UPLOAD_ERR_INI_SIZE) ? 'File too big' : '';

    // 이미지가 업로드되었거나 이미지 데이터를 가져와서 유효성 검사
    if ($temp and $image_error === UPLOAD_ERR_OK) {             // 파일이 업로드되었다면
        $article['image_alt'] = $_POST['image_alt'] ?? '';      // 대체 텍스트 가져오기

        // 이미지 파일 유효성 검사
        $errors['image_file'] = in_array(mime_content_type($temp), $file_types)
            ? ''
            : 'Wrong file type.';                               // 파일 타입 검사

        $ext = strtolower(pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION)); // 파일 확장자
        $errors['image_file'] .= in_array($ext, $file_exts)
            ? ''
            : 'Wrong file extension.';                          // 확장자 검사

        $errors['image_file'] .= ($_FILES['image']['size'] <= $max_size)
            ? ''
            : 'File too big.';                                  // 크기 검사

        $errors['image_alt'] = (is_text($article['image_alt'], 1, 254))
            ? ''
            : 'Alt text must be 1-254 characters';              // 대체 텍스트 검사

        // 이미지 파일이 유효하다면 저장할 위치 지정
        if ($errors['image_file'] === '' and $errors['image_alt'] === '') { // 유효하다면
            $article['image_file'] = create_filename($_FILES['image']['name'], $uploads);
            $destination = $uploads . $article['image_file'];    // 목적지
        }
    }

    // 기사 데이터 가져오기
    $article['title']       = $_POST['title'] ?? '';            // 제목
    $article['summary']     = $_POST['summary'] ?? '';          // 요약
    $article['content']     = $_POST['content'] ?? '';          // 내용
    $article['member_id']   = $_POST['member_id'] ?? 0;         // 저자
    $article['category_id'] = $_POST['category_id'] ?? 0;       // 카테고리
    $article['published']   = (isset($_POST['published'])
        and ($_POST['published'] == 1)) ? 1 : 0;                // 게시되었는가?

    // 기사 데이터 유효성 검사하고 유효하지 않다면 오류 메시지 생성
    $errors['title'] = is_text($article['title'], 1, 80)
        ? ''
        : 'Title must be 1-80 characters';

    $errors['summary'] = is_text($article['summary'], 1, 254)
        ? ''
        : 'Summary must be 1-254 characters';

    $errors['content'] = is_text($article['content'], 1, 100000)
        ? ''
        : 'Article must be 1-100,000 characters';

    $errors['author'] = is_member_id($article['member_id'], $authors)
        ? ''
        : 'Please select an author';

    $errors['category'] = is_category_id($article['category_id'], $categories)
        ? ''
        : 'Please select a category';

    $invalid = implode($errors);                                // 오류 결합하기

    // 파트 C: 데이터가 유효한지 확인하고, 유효하다면 데이터베이스 업데이트하기
    if ($invalid) {                                             // 유효하지 않다면
        $errors['warning'] = 'Please correct the errors below';  // 메시지 저장
    } else {                                                     // 그렇지 않다면
        $arguments = $article;                                  // 기사 데이터 저장

        try {                                                    // 데이터 삽입 시도
            $pdo->beginTransaction();                            // 트랜잭션 시작

            if ($destination) {                                  // 유효한 이미지가 있다면
                move_uploaded_file($temp, $destination);         // 파일 저장
                $sql = "INSERT INTO image (file, alt)
                        VALUES (:file, :alt);";                 // 이미지를 추가하는 SQL

                pdo($pdo, $sql, [
                    'file' => $arguments['image_file'],
                    'alt'  => $arguments['image_alt'],
                ]);                                             // 이미지 추가

                $arguments['image_id'] = $pdo->lastInsertId();   // 새로운 이미지 아이디 얻기
            }

            // article 테이블에는 image_file, image_alt 컬럼이 없으므로 제거
            unset($arguments['image_file'], $arguments['image_alt']);

            if ($id) {
                $sql = "UPDATE article
                           SET title = :title, summary = :summary, content = :content,
                               category_id = :category_id, member_id = :member_id,
                               image_id = :image_id, published = :published
                         WHERE id = :id;";                      // 기사 업데이트하는 SQL
            } else {
                unset($arguments['id']);                         // 아이디 제거

                $sql = "INSERT INTO article (title, summary, content, category_id,
                                             member_id, image_id, published)
                        VALUES (:title, :summary, :content, :category_id, :member_id,
                                :image_id, :published);";       // 기사를 생성하는 SQL
            }

            pdo($pdo, $sql, $arguments);                         // 기사를 추가하는 SQL 실행
            $pdo->commit();                                      // 변경사항 커밋

            redirect('articles.php', ['success' => 'Article saved']); // 리디렉션
        } catch (PDOException $e) {                              // PDOException이 발생했다면
            $pdo->rollBack();                                    // SQL 변경사항 롤백

            if ($destination && file_exists($destination)) {      // 이미지 파일이 존재한다면
                unlink($destination);                            // 이미지 파일 삭제
            }

            if (($e->errorInfo[1] ?? null) === 1062) {           // 무결성 제약에 걸렸다면
                $errors['warning'] = 'Article title already used'; // 경고문 저장
            } else {                                             // 그렇지 않다면
                throw $e;                                        // 예외 다시 발생
            }
        }
    }

    // 새로운 이미지가 업로드되었지만 데이터가 유효하지 않다면, article에서 이미지 제거
    $article['image_file'] = $saved_image ? $article['image_file'] : '';
}

include '../includes/admin-header.php';
?>

<form action="article.php?id=<?= $id ?>" method="POST" enctype="multipart/form-data">
    <main class="container admin" id="content">
        <h1>Edit Article</h1>

        <?php if ($errors['warning']) { ?>
            <div class="alert alert-danger"><?= $errors['warning'] ?></div>
        <?php } ?>

        <div class="admin-article">
            <section class="image">
                <?php if (!$article['image_file']) { ?>
                    <label for="image">Upload image:</label>
                    <div class="form-group image-placeholder">
                        <input type="file" name="image" class="form-control-file" id="image"><br>
                        <span class="errors"><?= $errors['image_file'] ?></span>
                    </div>

                    <div class="form-group">
                        <label for="image_alt">Alt text: </label>
                        <input type="text" name="image_alt" id="image_alt"
                               value="<?= html_escape($article['image_alt']) ?>" class="form-control">
                        <span class="errors"><?= $errors['image_alt'] ?></span>
                    </div>
                <?php } else { ?>
                    <label>Image:</label>
                    <img src="../uploads/<?= html_escape($article['image_file']) ?>"
                         alt="<?= html_escape($article['image_alt']) ?>">

                    <p class="alt">
                        <strong>Alt text:</strong> <?= html_escape($article['image_alt']) ?>
                    </p>

                    <a href="alt-text-edit.php?id=<?= $article['id'] ?>" class="btn btn-secondary">Edit alt text</a>
                    <a href="image-delete.php?id=<?= $id ?>" class="btn btn-secondary">Delete image</a><br><br>
                <?php } ?>
            </section>

            <section class="text">
                <div class="form-group">
                    <label for="title">Title: </label>
                    <input type="text" name="title" id="title"
                           value="<?= html_escape($article['title']) ?>" class="form-control">
                    <span class="errors"><?= $errors['title'] ?></span>
                </div>

                <div class="form-group">
                    <label for="summary">Summary: </label>
                    <textarea name="summary" id="summary" class="form-control"><?= html_escape($article['summary']) ?></textarea>
                    <span class="errors"><?= $errors['summary'] ?></span>
                </div>

                <div class="form-group">
                    <label for="content">Content: </label>
                    <textarea name="content" id="content" class="form-control"><?= html_escape($article['content']) ?></textarea>
                    <span class="errors"><?= $errors['content'] ?></span>
                </div>

                <div class="form-group">
                    <label for="member_id">Author: </label>
                    <select name="member_id" id="member_id">
                        <?php foreach ($authors as $author) { ?>
                            <option value="<?= $author['id'] ?>"
                                <?= ($article['member_id'] == $author['id']) ? 'selected' : ''; ?>>
                                <?= html_escape($author['forename'] . ' ' . $author['surname']) ?>
                            </option>
                        <?php } ?>
                    </select>
                    <span class="errors"><?= $errors['author'] ?></span>
                </div>

                <div class="form-group">
                    <label for="category">Category: </label>
                    <select name="category_id" id="category">
                        <?php foreach ($categories as $category) { ?>
                            <option value="<?= $category['id'] ?>"
                                <?= ($article['category_id'] == $category['id']) ? 'selected' : ''; ?>>
                                <?= html_escape($category['name']) ?>
                            </option>
                        <?php } ?>
                    </select>
                    <span class="errors"><?= $errors['category'] ?></span>
                </div>

                <div class="form-check">
                    <input type="checkbox" name="published" value="1"
                           class="form-check-input" id="published"
                        <?= ($article['published'] == 1) ? 'checked' : ''; ?>>
                    <label for="published" class="form-check-label">Published</label>
                </div>

                <input type="submit" name="update" value="Save" class="btn btn-primary">
            </section>
        </div>
    </main>
</form>

<?php include '../includes/admin-footer.php'; ?>